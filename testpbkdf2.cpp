...
{
  Password pwd;
  pwd.generate(PasswordParam("ct.de", "test", "abcdefghijklmnopqrstuvwxyzABCDEFGHJKLMNPQRTUVWXYZ0123456789#!\"§$%&/()[]{}=-_+*<>;:.", 10, 4096));
...
{
  Password pwd;
  pwd.generate(PasswordParam("MyFavoriteDomain", "foobar", "abcdefghijklmnopqrstuvwxyzABCDEFGHJKLMNPQRTUVWXYZ", 16, 8192));
...
{
  Password pwd;
  pwd.generate(PasswordParam("Bank", "reallysafe", "0123456789", 4, 1));
...
{
  Password pwd;
  pwd.generate(PasswordParam(QByteArray(), "cttest", "0123456789", 13, 4096));
