...
void TestPBKDF2::simple1(void)
{
  ...
  bool doQuit = false;
  qreal elapsed = 0;
  pwd.generate("ct.de", "pepper", "test", "abcdefghijklmnopqrstuvwxyzABCDEFGHJKLMNPQRTUVWXYZ0123456789#!\"§$%&/()[]{}=-_+*<>;:.", 10, 4096, doQuit, &elapsed);
...
void TestPBKDF2::simple2(void)
{
  ...
  bool doQuit = false;
  qreal elapsed = 0;
  pwd.generate("MyFavoriteDomain", "pepper", "foobar", "abcdefghijklmnopqrstuvwxyzABCDEFGHJKLMNPQRTUVWXYZ", 16, 8192, doQuit, &elapsed);
...
void TestPBKDF2::pin(void)
{
  ...
  bool doQuit = false;
  qreal elapsed = 0;
  pwd.generate("Bank", "pepper", "reallysafe", "0123456789", 4, 1, doQuit, &elapsed);
