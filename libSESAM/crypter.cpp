 ...
 *
 */
std::random_device Crypter::randomDev;
#endif
 ...
 */
QByteArray Crypter::randomBytes(const int size, bool *ok)
{
  QByteArray buf(size, '\0');
#ifdef Q_OS_WIN
  bool useFallback = !isRdRandSupported();
  if (!useFallback) {
  ...
  if (useFallback) {
    ...
    if (!CryptAcquireContextW(&hProvider,
      ...
      if (!CryptGenRandom(hProvider, size, reinterpret_cast<BYTE*>(buf.data()))) {
  ...
  for (int i = 0; i < size; ++i) {
    buf[i] = static_cast<char>(randomDev());
  }
#endif
  return buf;
}
